...
/**
 * build_svellheim_imbuings_pack.js
 *
 * Reads all Imbuing Project JSON files from
 * campaign/foundry-json-exports/Imbuing Projects/
 * and writes them into a LevelDB compendium pack with Foundry folder structure.
 *
 * Directory layout expected:
 *   Imbuing Projects/
 *     Armor/      1st-Level/  5th-Level/  9th-Level/
 *     Implement/  1st-Level/  5th-Level/  9th-Level/
...
// Category sort order: Armor < Implement < Weapon
// Level sort order: 1st < 5th < 9th

const CATEGORY_SORT = { 'Armor': 100000, 'Implement': 200000, 'Weapon': 300000 };
...
function buildFolders() {
  ...
  // Root folder
  const rootId = foundryId('svi-folder:root');
  folders.push(mkFolder({ id: rootId, name: 'Imbuing Projects', sort: 0 }));

  // Category + level folders
  for (const [cat, catSort] of Object.entries(CATEGORY_SORT)) {
    const catId = foundryId(`svi-folder:${cat.toLowerCase()}`);
    folders.push(mkFolder({ id: catId, name: cat, parentId: rootId, sort: catSort }));

    for (const [lvl, lvlSort] of Object.entries(LEVEL_SORT)) {
      const lvlId = foundryId(`svi-folder:${cat.toLowerCase()}:${lvl}`);
  ...
  for (const cat of Object.keys(CATEGORY_SORT)) {
    for (const lvl of Object.keys(LEVEL_SORT)) {
      ...
      for (const file of jsonFiles) {
        ...
        item._id    = foundryId(`svi-item:${dsid}`);
...
async function main() {
  ...
  console.log(`Found ${items.length} imbuing items across ${folders.length} folders`);

  await writeLevelDb({ folders, items }, PACK_DIR);

  const byEchelon = {};
  for (const i of items) {
    const e = i.system?.echelon || '?';
    byEchelon[`echelon ${e}`] = (byEchelon[`echelon ${e}`] || 0) + 1;
  }

  console.log(`\nWrote ${items.length} items and ${folders.length} folders to ${path.relative(REPO_ROOT, PACK_DIR) + '/'}`);
  if (Object.keys(byEchelon).length) {
    console.log('By echelon:');
    for (const [e, c] of Object.entries(byEchelon).sort()) console.log(`  ${e}: ${c}`);
  }
}
